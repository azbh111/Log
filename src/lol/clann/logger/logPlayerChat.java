...
public class logPlayerChat extends LoggerListener {
    ...
    public static logPlayerChat register() throws SQLException {
        return new logPlayerChat();
    }

    public logPlayerChat() throws SQLException {
        super();
    }
    ...
    public void onPlayerChat(PlayerChatEvent e) {
        try {
            logPlayerChat(e.getPlayer(), e.getMessage());
        } catch (SQLException ex) {
            API.log(ex, "PlayerChatEvent异常");
        }
    }

    private void logPlayerChat(Player p, String message) throws SQLException {
    ...
    protected String getTableColumnDefinition() {
        return "time bigint,tick int,player int,message text";
    }

    @Override
    protected void createIndex() throws SQLException {
        Log.plugin.sql.getStatement().execute("create clustered index pcIdx_time on " + name + " (time)");//日期聚集索引
        Log.plugin.sql.getStatement().execute("create nonclustered index pcIdx_player on " + name + " (player)");//玩家非聚集索引
    }
